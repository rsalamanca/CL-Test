...
$(window).on("load", function() {
   $(function() {
      ...
      verticalLineChart(chart.options.data[1].dataPoints);
      ...
      function verticalLineChart(arrayDataPoints) {
         var axisY = [];
         var axisX = [];
         var dataInsert = [];
         ...
         for (var i = 0; i < arrayDataPoints.length; i++) {
            ...
            chart.options.data.push(dataInsert[i]);
         }
      };
      //datapoint Hover

      function test(e) {
         var className = '.' + e.dataPoint.name;
         var leftX = event.pageX - ($(className).outerWidth() / 2);
         var topY = event.pageY - $(className).outerHeight() - 40;
         $(className).css({
            top: topY,
            left: leftX
         }).show('fast');
      ...
      var handleHelper = scrollbar.find(".ui-slider-handle")
         .append("<span class='ui-icon ui-icon-grip-dotted-vertical noUi-handle'><span class='verticalLines'>lorem</span></span>")
